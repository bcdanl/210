...
- Tips for Collecting Data from [Yahoo! Finance](https://finance.yahoo.com)’s Sustainability Section:
  1.	To avoid getting blocked for automated activity, use `time.sleep(random.uniform(x, y))` between page visits.
	2.	Use explicit waits to ensure elements load before scraping.
...
### 2. `yfinance`
-	`yfinance` is an unofficial API for retrieving financial data from [Yahoo! Finance](https://finance.yahoo.com).
-	For each company in the `esg_proj_2025` DataFrame, collect the following:
