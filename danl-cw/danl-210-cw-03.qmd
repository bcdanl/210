...
2. Add a drop-down menu of "Python Data Analysis" to the navigation bar.
  - Under the menu of "Python Data Analysis", add links for the following three webpages:
    a. **Python Basics** using `basic-python-intro.ipynb`
    b. **Python Basics with Pandas** using `danl-210-py-only.qmd`
    c. **Using Python and R together!** using `danl-210-quarto-reticulate.qmd`
    
3. Add a blog post that explains Python Basics to your blog.
  - Start with creating a directory in the `posts` directory.
  - One way is using `qmd` file to write the blog post directly with the following YAML.


4. Use the 3-step git commands (`git add`, `git commit`, and `git push`) to update your website.
