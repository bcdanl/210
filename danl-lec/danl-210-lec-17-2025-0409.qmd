...
## (4)
- `response.content.decode('utf-8')`: Extracts the content (i.e. data) from the `response`.
- `json.loads()`: Converts JSON into a Python dictionary object.
  
```{.python}
# Convert JSON response to Python dictionary.
content = response.json() # to parse JSON response data
