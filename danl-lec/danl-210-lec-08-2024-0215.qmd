---
title: Lecture 8
subtitle: Pandas Basics - Loading, Summarizing, Selecting, Sorting, Indexing, and Locating Data
format:
  clean-revealjs:
    self-contained: true
    incremental: true   
...
- Save your Jupyter Notebook for each class to a dedicated directory in Google Drive, local laptop, or a GitHub repo.
  - Go to **File** and select **Save ...**/
  ( e.g., `danl-210-lec-08-2024-0215.ipynb`)
...
- In your website project directory, avoid having 
  ...
  - `.qmd`/`.ipynb` files you do not use for your website.
...
1. Ensure all Quarto documents are rendered well without any errors.

2. After making edits to `_quarto.yml`, save the changes by clicking the floppy disk icon (💾).

3. Run `quarto render` in Terminal, whenever you have some changes in `_quarto.yml`.

4. Once `quarto render` completes, view the `index.html` in your website working directory to see the updates from a web browser.
