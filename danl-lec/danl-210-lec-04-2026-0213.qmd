...
# Ethics First: Scraping is Not “Free” {background-color="#1c4982"}
...
- Respect **website owners’ Terms of Service (ToS)**.
...
## A Simple “Responsible Scraping” Checklist ✅
...
## Good Scraping Behavior (What “Polite” Looks Like)
...
- Don’t scrape behind logins/paywalls unless you have explicit permission.
...
- **JavaScript** runs in the browser and can:
  - load more data after the page “finishes” loading,
...
## Cascading Style Sheets (CSS) — and why scrapers still care
...
- If the page is static:
  - prefer `requests + BeautifulSoup` (faster, simpler, more “polite”).
- Use Selenium when:
...
## 1) CSS selectors = *target the right element* 🎯

- Selenium doesn’t “read the page like a human” — it selects **DOM elements**.
- **CSS selectors** are a compact way to say: *“find this element in the HTML.”*
- Reliability tip:
  - Prefer stable identifiers: `#id`, unique classes, meaningful attributes.
  - Avoid fragile paths that break when layout changes.

::: {.callout-note}
**Ethics angle:** Accurate targeting means you collect **only what you need** (data minimization) and avoid “scrape-everything” behavior.
:::

## 2) Explicit waits = *don’t fight the website* ⏳
...
::: {.callout-note}
**Ethics angle:** Waiting properly reduces unnecessary requests and prevents accidental “hammering” caused by retry loops.
:::

## 3) Rate limiting = *be polite to the server* 🧑‍🤝‍🧑
...
::: {.callout-warning}
**Ethics angle:** Rate limiting is about **respecting shared infrastructure**—your script should not degrade service for real users.
:::
...
## 1) XPath = *target the right element* 🎯

- Selenium doesn’t “read the page like a human” — it selects **DOM elements**.
- **XPaths** are a compact way to say: *“find this element in the HTML.”*
- Reliability tip:
  - Prefer stable identifiers: `#id`, unique classes, meaningful attributes.
  - Avoid fragile paths that break when layout changes.
